...
#create boundaries
for j in range(length):
    coords[j][0][2]=0#40*sin(j*pi/(length-1))
...
while True:
    ...
    for j in range(1,length-1):
        ...
        #along j=0
        i=j
        u0=coords[i][length-1][2]
        u1=coords[i+1][length-1][2]
        u2=coords[i-1][length-1][2]
        u3=coords[i][length-2][2]
        fu=k*(u2-u0)+k*(u1-u0)+k*(u3-u0)
        m=coords[i][length-1][6]
        vu=coords[i][length-1][5]
        a=scale*fu/m
        vu+=a*dt
        u0+=vu*dt
        coords[i][length-1][2]=u0
        bereal=5
    ...
    #pulse middle point
    coords[round(length/2)][round(length/2)][2]=200#240*sin(t*.1)
